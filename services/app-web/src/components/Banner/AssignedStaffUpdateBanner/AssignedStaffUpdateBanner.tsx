...
export const AssignedStaffUpdateBanner = ({
    ...
    removed,
    added,
}: LastUpdatedAnalystsType & React.HTMLAttributes<HTMLDivElement>): React.ReactElement | null => {
    if (!state || (!removed && !added)){
        recordJSException(`AssignedStaffUpdate display not possible, bad data. State: ${state}, Added: ${added}, Removed: ${removed}`)
        return null
    }
    return (
        <Alert
