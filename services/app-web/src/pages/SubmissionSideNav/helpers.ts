...
const isUnlockedOrDraft = (
    submissionStatus: 'UNLOCKED' | 'DRAFT' | 'SUBMITTED' | 'RESUBMITTED'
) => submissionStatus === 'UNLOCKED' || submissionStatus === 'DRAFT'

const shouldUseFormPageStyles = (
    routeName: RouteTWithUnknown,
    user: User,
    isEditablePage: boolean
) => {
    const isStateUser = user.role === 'STATE_USER'
    return (
        (STATE_SUBMISSION_FORM_ROUTES.includes(routeName) &&
