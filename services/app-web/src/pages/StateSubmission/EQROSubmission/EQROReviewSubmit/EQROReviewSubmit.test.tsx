...
it('uses SUBMIT_CHIP_ONLY modal for CHIP-only EQRO draft when flag is on', async () => {
    const testData = testEQROContract(
        { id: 'test-id' },
    ...
    renderWithProviders(
        ...
        </Routes>,
        {
            apolloProvider: {
                mocks: [
                    fetchCurrentUserMock({ statusCode: 200 }),
                    fetchContractMockSuccess({ contract: testData }),
                ],
            },
            routerProvider: {
                route: '/submissions/eqro/test-id/edit/review-and-submit',
...
it('uses RESUBMIT_CHIP_ONLY modal for unlocked CHIP-only EQRO when flag is on', async () => {
    const testData = testEQROContract(
        { id: 'test-id', status: 'UNLOCKED' },
        { populationCovered: 'CHIP' }
    )
    ...
    // RESUBMIT_CHIP_ONLY reuses the CHIP-only description but adds a textarea
    expect(
        await screen.findByText(
            'Once you submit, your submission will be locked. CMS must unlock it before you can edit your responses.'
        )
