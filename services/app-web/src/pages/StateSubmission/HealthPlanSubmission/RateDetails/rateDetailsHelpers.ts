...
import { type FormikRateForm } from './RateDetails'
...
const isRatePartiallyFilled = (rate: RateFormData): boolean => {
    const hasActuaryContacts = (actuaries: ActuaryContact[]): boolean =>
        actuaries.some(
            (actuary) =>
                actuary.id ||
                actuary.name ||
...
const convertIndexRatesGQLRateToRateForm = (
    getKey: S3ClientT['getKey'],
    rate?: RateWithoutContractData,
    parentContractID?: string
): FormikRateForm => {
    const handleAsLinkedRate =
        rate && rate.parentContractID !== parentContractID // TODO: Make this a more sophisticated check for child-rates
    const rateRev = handleAsLinkedRate
        ? rate?.revisions[0]
        : rate?.draftRevision
    ...
    return {
        ...
        ratePreviouslySubmitted: handleAsLinkedRate
            ? 'YES'
            : isFilledIn
              ? 'NO'
              : undefined,
...
// if rate is not passed in, return an empty RateForm // we need to pass in the  s3 handler because 3 urls generated client-side
// useLatestSubmission means to pull the latest submitted info rather than the draft info
const convertGQLRateToRateForm = (
    getKey: S3ClientT['getKey'],
    rate?: Rate,
    parentContractID?: string
): FormikRateForm => {
