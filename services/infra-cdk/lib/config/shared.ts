...
export class ResourceNames {
    ...
    static resourceName = (
        ...
        resource: string,
        stage: string
    ): string => `${PROJECT_PREFIX}-${stage}-${service}-${resource}`
    static ssmPath = (path: string, stage: string): string =>
        path.replace('{stage}', stage)
    static apiName = (service: string, stage: string): string =>
        `${PROJECT_PREFIX}-${stage}-${service}`
    static databaseName = (stage: string, account: string): string =>
        `aurorapostgres${stage}${account}cdk`
    static ssmParameterName = (
        category: string,
        key: string,
        stage: string
    ): string => `/${PROJECT_PREFIX}/${stage}/${category}/${key}`
}
