...
export class BuildConfig extends Construct {
    ...
    public getConfiguration(applicationEndpoint: string): BuildConfiguration {
        ...
        const identityPoolId = ServiceRegistry.getIdentityPoolId(
            this,
            this.stage
        )
        const userPoolId = ServiceRegistry.getUserPoolId(this, this.stage)
        const userPoolClientId = ServiceRegistry.getUserPoolClientId(
            this,
            this.stage
        )
        ...
        const documentsBucket = ServiceRegistry.getS3BucketName(
            this,
            this.stage,
            'uploads'
        )
        const qaBucket = ServiceRegistry.getS3BucketName(
            this,
            this.stage,
            'qa-uploads'
        )
        ...
        const ldClientId = this.getOptionalSsmParameter(
            '/configuration/react_app_ld_client_id_feds'
        )
        const nrAccountId = this.getOptionalSsmParameter(
            '/configuration/react_app_nr_account_id'
        )
        const nrAgentId = this.getOptionalSsmParameter(
            '/configuration/react_app_nr_agent_id'
        )
        const nrLicenseKey = this.getOptionalSsmParameter(
            '/configuration/react_app_nr_license_key'
        )
        const nrTrustKey = this.getOptionalSsmParameter(
            '/configuration/react_app_nr_trust_key'
        )
