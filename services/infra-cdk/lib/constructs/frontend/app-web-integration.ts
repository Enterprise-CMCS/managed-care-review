...
export class AppWebIntegration extends Construct {
    ...
    constructor(scope: Construct, id: string, props: AppWebIntegrationProps) {
        ...
        if (!mainAppExists || !storybookExists) {
            console.info(
                'AppWebIntegration: Build directories not found. Please run the following commands:'
            )
            console.info(`  cd ${appWebPath}`)
            console.info('  pnpm install')
            console.info('  pnpm build')
            console.info('  pnpm storybook:build')
            console.info('')
            console.info('For CDK synth, creating placeholder deployments...')
        }
        ...
        this.mainAppHtmlDeployment = new s3deploy.BucketDeployment(
            this,
            'MainAppHtmlDeployment',
            {
                sources: mainAppExists
                    ...
                    : [
                          s3deploy.Source.data(
                              'index.html',
                              '<html><body>Placeholder - run build first</body></html>'
                          ),
                      ],
        ...
        this.mainAppAssetsDeployment = new s3deploy.BucketDeployment(
            this,
            'MainAppAssetsDeployment',
            {
                sources: mainAppExists
                    ? [
                          s3deploy.Source.asset(mainAppBuildPath, {
                ...
                destinationKeyPrefix: assetPrefix,
                cacheControl:
                    props.enableImmutableAssets !== false
                        ? [
        ...
        this.mainAppImagesDeployment = new s3deploy.BucketDeployment(
            this,
            'MainAppImagesDeployment',
            {
                sources: mainAppExists
                    ? [
                          s3deploy.Source.asset(mainAppBuildPath, {
        ...
        this.storybookHtmlDeployment = new s3deploy.BucketDeployment(
            this,
            'StorybookHtmlDeployment',
            {
                sources: storybookExists
                    ...
                    : [
                          s3deploy.Source.data(
                              'index.html',
                              '<html><body>Storybook placeholder - run build first</body></html>'
                          ),
                      ],
        ...
        this.storybookAssetsDeployment = new s3deploy.BucketDeployment(
            this,
            'StorybookAssetsDeployment',
            {
                sources: storybookExists
                    ? [
                          s3deploy.Source.asset(storybookBuildPath, {
        ...
        this.storybookImagesDeployment = new s3deploy.BucketDeployment(
            this,
            'StorybookImagesDeployment',
            {
                sources: storybookExists
                    ? [
                          s3deploy.Source.asset(storybookBuildPath, {
    ...
    private updateCloudFrontOriginPath(props: AppWebIntegrationProps): void {
        ...
        console.info(
            `Deploying version ${props.commitSha?.substring(0, 8)} to ${props.stage}`
        )
    }
    ...
    private addDeploymentMetadata(stage: string, commitSha?: string): void {
        ...
        allDeployments.forEach((deployment) => {
            deployment.node.addMetadata(
                'deployment-source',
                'cdk-app-web-integration'
            )
