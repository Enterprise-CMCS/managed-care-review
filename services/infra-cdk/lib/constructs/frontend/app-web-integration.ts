...
export interface AppWebIntegrationProps {
  ...
  /**
   * The S3 bucket for storybook (optional - can be in separate stack)
   */
  ...
  /**
   * The CloudFront distribution for storybook (optional - can be in separate stack)
   */
  ...
  /**
   * Enable unified Storybook deployment to same bucket/distribution as main app
   * When true, deploys Storybook to /storybook/ path in main app bucket
   * Alternative to separate storybookBucket/storybookDistribution
   */
  enableUnifiedStorybookDeployment?: boolean;
}
...
export class AppWebIntegration extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AppWebIntegrationProps) {
    ...
    if (!mainAppExists) {
      console.log('AppWebIntegration: Main app build directory not found. Please run:');
    ...
    // Only deploy storybook if bucket and distribution are provided
    if (props.storybookBucket && props.storybookDistribution) {
      if (!storybookExists) {
        console.log('AppWebIntegration: Storybook build directory not found. Please run:');
        console.log(`  cd ${appWebPath}`);
    ...
    // Deploy storybook to main app bucket (unified deployment approach)
    if (props.enableUnifiedStorybookDeployment && !props.storybookBucket) {
      if (!storybookExists) {
        console.log('AppWebIntegration: Storybook build directory not found. Please run:');
        console.log(`  cd ${appWebPath}`);
        console.log('  pnpm storybook:build');
      ...
      // Deploy storybook HTML files to /storybook/ path in main app bucket
      this.storybookHtmlDeployment = new s3deploy.BucketDeployment(this, 'UnifiedStorybookHtmlDeployment', {
        sources: storybookExists
          ? [s3deploy.Source.asset(storybookBuildPath, {
              exclude: ['**/*.js', '**/*.css', '**/*.woff', '**/*.woff2', '**/*.ttf', '**/*.eot', '**/*.jpg', '**/*.jpeg', '**/*.png', '**/*.gif', '**/*.svg', '**/*.ico', '**/*.map']
            })]
          : [s3deploy.Source.data('index.html', '<html><body>Storybook placeholder - run build first</body></html>')],
        destinationBucket: props.mainAppBucket,
        destinationKeyPrefix: 'storybook/',  // Deploy to /storybook/ path
        distribution: props.mainAppDistribution,
        distributionPaths: ['/storybook/*'],  // Invalidate storybook paths only
        cacheControl: [s3deploy.CacheControl.noCache()],
        memoryLimit: 512,
        ephemeralStorageSize: Size.gibibytes(1),
        prune: false,
        ...
        metadata: {
          'deployment-stage': props.stage,
          'deployment-source': 'cdk-native-unified',
          'deployment-timestamp': new Date().toISOString()
        }
      });
      
      // Deploy storybook assets to /storybook/ path with immutable caching
      this.storybookAssetsDeployment = new s3deploy.BucketDeployment(this, 'UnifiedStorybookAssetsDeployment', {
        sources: storybookExists 
          ? [s3deploy.Source.asset(storybookBuildPath, {
              exclude: ['**/*.html']  // Only assets, not HTML
            })]
          : [],
        destinationBucket: props.mainAppBucket,
        destinationKeyPrefix: 'storybook/',  // Deploy to /storybook/ path
        distribution: props.mainAppDistribution,
        distributionPaths: ['/storybook/*'],  // Invalidate storybook paths only
        cacheControl: props.enableImmutableAssets ? [s3deploy.CacheControl.immutable()] : [s3deploy.CacheControl.maxAge(Duration.days(7))],
        memoryLimit: 512,
        ephemeralStorageSize: Size.gibibytes(1),
        prune: false,
        retainOnDelete: false,
  ...
  private addDeploymentMetadata(stage: string): void {
    ...
    const allDeployments = [
      ...
      ...(this.storybookHtmlDeployment ? [this.storybookHtmlDeployment] : []),
      ...(this.storybookAssetsDeployment ? [this.storybookAssetsDeployment] : []),
      ...(this.storybookImagesDeployment ? [this.storybookImagesDeployment] : [])
    ];
