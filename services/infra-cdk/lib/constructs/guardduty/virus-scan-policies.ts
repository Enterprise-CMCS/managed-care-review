...
export class VirusScanPolicies extends Construct {
    constructor(scope: Construct, id: string, props: VirusScanPoliciesProps) {
        ...
        const allowedExtensions = props.allowedFileExtensions || [
            ...S3_DEFAULTS.ALLOWED_FILE_EXTENSIONS,
        ]
     ...
     */
    private addDenyUnscannedFileAccessPolicy(
        bucket: s3.IBucket,
     ...
     */
    private addFileTypeRestrictionPolicy(
        bucket: s3.IBucket,
        ...
        const policy = new iam.PolicyStatement({
            ...
            notResources: allowedExtensions.map(
                (ext) => `${bucket.bucketArn}/${ext}`
            ),
        })
