...
import { type StageConfig } from '@config/index'
...
export class RescanCapability extends Construct {
    ...
    private createRescanHandler(
        props: RescanCapabilityProps
    ): BaseLambdaFunction {
    ...
    private createFailedScanRule(props: RescanCapabilityProps): void {
        this.failedScanRule = new events.Rule(this, 'FailedScanRule', {
            ...
            description:
                'Trigger on failed GuardDuty scans for potential rescan',
            eventPattern: {
        ...
        this.failedScanRule.addTarget(
            new targets.LambdaFunction(this.rescanHandler.function, {
                retryAttempts: 2,
            })
        )
    }
