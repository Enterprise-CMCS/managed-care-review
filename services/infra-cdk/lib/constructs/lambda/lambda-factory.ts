...
export class LambdaFactory extends Construct {
    ...
    public createFunction(
        ...
        if (this.functions.has(functionId)) {
            throw new Error(
                `Function ${functionId} already exists in this factory`
            )
        }
        ...
        if (!entry) {
            throw new Error(
                `No handler found for function: ${functionProps.functionName}`
            )
        }
