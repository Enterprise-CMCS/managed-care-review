                        ...
                        local: {
                            tryBundle(outputDir: string): boolean {
                                ...
                                try {
                                    ...
                                    console.info(
                                        `📦 Using ${pm} for Prisma layer build`
                                    )
                                    ...
                                    if (fs.existsSync(schemaPath)) {
                                        ...
                                        execSync(
                                            `PRISMA_CLI_BINARY_TARGETS=rhel-openssl-3.0.x ${pm} exec prisma generate`,
                                            {
                                                cwd: tempDir,
                                                stdio: 'inherit',
                                                env: {
                                    ...
                                    execSync(
                                        `find ${outputDir} -name "*.ts" -delete`,
                                        { stdio: 'ignore' }
                                    )
                                    execSync(
                                        `find ${outputDir} -name "*.md" -delete`,
                                        { stdio: 'ignore' }
                                    )
                                    execSync(
                                        `find ${outputDir} -name "test" -type d -exec rm -rf {} + 2>/dev/null || true`,
                                        { stdio: 'ignore' }
                                    )
                                    execSync(
                                        `find ${outputDir} -name ".github" -type d -exec rm -rf {} + 2>/dev/null || true`,
                                        { stdio: 'ignore' }
                                    )

                                    console.info(
                                        '✅ Prisma layer built successfully (Docker-free)'
                                    )
                                    return true // Tell CDK "skip Docker"
                                } catch (error: any) {
                                    console.error(
                                        '❌ Prisma layer build failed:',
                                        error?.message || error
                                    )
    ...
    public getEnvironment(): Record<string, string> {
        return {
            ...
            PRISMA_QUERY_ENGINE_LIBRARY:
                '/opt/nodejs/node_modules/.prisma/client/libquery_engine-rhel-openssl-3.0.x.so.node', // Match serverless - x86_64
