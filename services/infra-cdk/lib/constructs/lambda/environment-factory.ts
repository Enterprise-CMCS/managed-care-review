...
import { LAMBDA_COMMON_ENV, getOtelEnvironment } from '@config/constants'
...
export class LambdaEnvironmentFactory {
    ...
    static createBaseEnvironment(
        ...
        return {
            ...
            NODE_PATH:
                '/opt/nodejs/node_modules:/var/runtime/node_modules:/var/task/node_modules',
            ...LAMBDA_COMMON_ENV,
        }
    }
    ...
    static createOtelEnvironment(stage: string): OtelEnvironment {
        ...
        return {
            AWS_LAMBDA_EXEC_WRAPPER:
                otelVars.AWS_LAMBDA_EXEC_WRAPPER || '/opt/otel-handler',
            OPENTELEMETRY_COLLECTOR_CONFIG_FILE:
                otelVars.OPENTELEMETRY_COLLECTOR_CONFIG_FILE ||
                '/var/task/collector.yml',
            VITE_APP_OTEL_COLLECTOR_URL:
                otelVars.VITE_APP_OTEL_COLLECTOR_URL || '',
            OTEL_SERVICE_NAME: otelVars.OTEL_SERVICE_NAME || `mcr-${stage}`,
            OTEL_PROPAGATORS:
                otelVars.OTEL_PROPAGATORS || 'tracecontext,baggage,xray',
        }
    }
    ...
    static createDatabaseLambdaEnvironment(
        ...
        const otelEnv = options?.includeOtel
            ? this.createOtelEnvironment(stage)
            : {}
    ...
    static createApiLambdaEnvironment(
        ...
        const baseEnv = this.createBaseEnvironment(stage, region)
        const dbEnv = this.createDatabaseEnvironment(
            config.databaseSecretArn,
            stage,
            region
        )
        const s3Env = this.createS3Environment(
            config.uploadsBucket,
            config.qaBucket
        )
        const authEnv = this.createAuthEnvironment()
        const featureEnv = this.createFeatureFlagEnvironment(
            config.ldSdkKey,
            config.parameterStoreMode
        )
    ...
    static createVirusScanEnvironment(
        ...
        return {
            ...
            ENABLE_CLAMAV_COMPATIBILITY: String(
                options?.enableClamAvCompatibility || false
            ),
