...
import { ResourceNames, CDK_DEPLOYMENT_SUFFIX } from '../../config';
...
export class AuroraServerlessV2 extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AuroraServerlessV2Props) {
    ...
    // Use CDK secret with same logical ID to maintain export compatibility
    this.secret = secretsmanager.Secret.fromSecretNameV2(this, 'DatabaseSecret', 
      `mcr-cdk-aurora-postgres-${props.stage}`
    );

    // Create parameter group for PostgreSQL 16.6
    ...
    if (props.databaseConfig.snapshotIdentifier) {
      // Restore from snapshot
      this.cluster = new rds.DatabaseClusterFromSnapshot(this, 'Cluster', {
        snapshotIdentifier: props.databaseConfig.snapshotIdentifier,
        engine: rds.DatabaseClusterEngine.auroraPostgres({
          version: rds.AuroraPostgresEngineVersion.VER_16_6
        }),
        // No credentials specified - uses the snapshot's existing credentials
        clusterIdentifier: ResourceNames.resourceName('database', 'cluster', props.stage) + CDK_DEPLOYMENT_SUFFIX,
        // Note: defaultDatabaseName is not used when restoring from snapshot
        vpc: props.vpc,
        ...
        storageEncryptionKey: kms.Alias.fromAliasName(this, 'RdsKey', 'alias/aws/rds'),
