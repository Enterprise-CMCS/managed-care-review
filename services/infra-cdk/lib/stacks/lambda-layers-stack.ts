...
export class LambdaLayersStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: LambdaLayersStackProps) {
        ...
        this.prismaEngineLayerVersion = this.createPrismaEngineLayer(
            props.stage
        )
        this.prismaEngineLayerVersionArn =
            this.prismaEngineLayerVersion.layerVersionArn

        // Create Prisma Migration layer with automatic versioning
        this.prismaMigrationLayerVersion = this.createPrismaMigrationLayer(
            props.stage
        )
        this.prismaMigrationLayerVersionArn =
            this.prismaMigrationLayerVersion.layerVersionArn

        // Create PostgreSQL tools layer
        this.postgresToolsLayerVersion = this.createPostgresToolsLayer(
            props.stage
        )
        this.postgresToolsLayerVersionArn =
            this.postgresToolsLayerVersion.layerVersionArn
        ...
        // Export ARNs for cross-stack reference using the ResourceNames pattern
        new CfnOutput(this, 'PrismaEngineLayerArn', {
            value: this.prismaEngineLayerVersionArn,
            exportName: `MCR-lambda-layers-${props.stage}-cdk-PrismaEngineLayerArn`,
        ...
        new CfnOutput(this, 'PrismaMigrationLayerArn', {
            ...
            exportName: `MCR-lambda-layers-${props.stage}-cdk-PrismaMigrationLayerArn`,
        ...
        new CfnOutput(this, 'PostgresToolsLayerArn', {
            ...
            exportName: `MCR-lambda-layers-${props.stage}-cdk-PostgresToolsLayerArn`,
    ...
    private createPostgresToolsLayer(stage: string): lambda.LayerVersion {
        ...
        return new lambda.LayerVersion(this, 'PostgresToolsLayer', {
            ...
            // Use pre-built, size-optimized layer assets
            code: lambda.Code.fromAsset(
                path.join(__dirname, '../../lambda-layers-postgres-tools'),
                {
                    // No bundling - use pre-built artifacts from CI
                    // This prevents inline building which was causing 250MB+ layer sizes
                }
    ...
    private calculateLayerHash(layerType: string, stage: string): string {
        ...
        const mainBuildScriptPath = path.join(
            __dirname,
        ...
        try {
            if (fs.existsSync(mainBuildScriptPath)) {
                ...
                if (stage === 'dev') {
                    console.info(
                        `Layer ${layerType}: Including main build script content (${scriptContent.length} chars)`
                    )
                }
            }
        } catch (error) {
            console.warn(
                `Layer hash: Could not read main build script ${mainBuildScriptPath}, skipping content hash: ${error}`
            )
        }
        ...
        if (layerType.includes('prisma')) {
            ...
            if (stage === 'dev') {
                console.info(
                    `Layer ${layerType}: Using Prisma version for deterministic hash`
                )
            }
        }
