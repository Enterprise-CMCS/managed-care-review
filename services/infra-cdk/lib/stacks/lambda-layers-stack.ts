...
export class LambdaLayersStack extends Stack {
    ...
    private createPostgresToolsLayer(stage: string): lambda.LayerVersion {
        ...
        const buildScriptPath = path.join(
            __dirname,
            '../../lambda-layers-postgres-tools/build.sh'
        )
        const layerPath = path.join(
            __dirname,
            '../../lambda-layers-postgres-tools'
        )
        ...
        return new lambda.LayerVersion(this, 'PostgresToolsLayer', {
            ...
            code: lambda.Code.fromAsset(
                path.join(__dirname, '../constructs/lambda/layers'),
                {
                    bundling: {
                            ...
                            tryBundle(outputDir: string): boolean {
                                ...
                                try {
                                    ...
                                    const packageJson = {
                                        ...
                                        version: '1.0.0',
                                        description:
                                            'PostgreSQL client tools for Lambda',
                                        dependencies: {
                                            pg: '^8.11.3',
                                            'pg-format': '^1.0.4',
                                        },
                                    ...
                                    console.info(
                                        'PostgreSQL tools layer built successfully with local bundling'
                                    )
                                    return true
                                } catch (error) {
                                    console.error(
                                        'Failed to build PostgreSQL tools layer locally:',
                                        error
                                    )
                                    return false
                                }
