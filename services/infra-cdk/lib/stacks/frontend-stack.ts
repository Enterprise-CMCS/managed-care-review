...
import { BaseStack, ServiceRegistry } from '@constructs/base'
import {
    StaticWebsite,
    GeoRestrictedWaf,
    BuildConfig,
    AppWebIntegration,
} from '@constructs/frontend'
...
export class FrontendStack extends BaseStack {
    ...
    constructor(scope: Construct, id: string, props: FrontendStackProps) {
        super(scope, id, {
            ...props,
            description:
                'Frontend stack for Managed Care Review - Static website hosting with CloudFront',
        })
    ...
    protected defineResources(): void {
        ...
        this.mainApp = new StaticWebsite(this, 'MainApp', {
            ...
            webAcl: waf.webAcl,
            customDomain:
                this.appDomainName && this.appCertificateArn
                    ? {
        ...
        this.storybook = new StaticWebsite(this, 'Storybook', {
            ...
            webAcl: waf.webAcl,
            customDomain:
                this.storybookDomainName && this.storybookCertificateArn
                    ? {
  ...
  return response;
    ...
    private storeServiceRegistryValues(): void {
        ...
        ServiceRegistry.putValue(
            ...
            'frontend',
            'ui-bucket-name',
            this.mainApp.bucket.bucketName,
            this.stage
        )
        ServiceRegistry.putValue(
            this,
            'frontend',
            'ui-distribution-id',
            this.mainApp.distribution.distributionId,
            this.stage
        )
        ServiceRegistry.putValue(
            this,
        ...
        ServiceRegistry.putValue(
            this,
            'frontend',
            'storybook-bucket-name',
            this.storybook.bucket.bucketName,
            this.stage
        )
        ServiceRegistry.putValue(
            this,
            'frontend',
            'storybook-distribution-id',
            this.storybook.distribution.distributionId,
            this.stage
        )
        ServiceRegistry.putValue(
            this,
    ...
    private grantDeploymentPermissions(): void {
        ...
        if (githubRoleArn) {
            try {
                const githubRole =
                    (this.node.root.node.findChild('GitHubRole') as iam.Role) ||
                    iam.Role.fromRoleArn(this, 'GitHubRole', githubRoleArn)
                ...
                console.info(
                    `GitHub OIDC role not found, skipping deployment permissions: ${error}`
                )
            }
        }
    }
