...
export class AuthStack extends BaseStack {
    ...
    constructor(scope: Construct, id: string, props: AuthStackProps) {
        super(scope, id, {
            ...props,
            description:
                'Auth stack for Managed Care Review - Cognito User Pool and Identity Pool',
        })
    ...
    private validateConfiguration(): void {
        ...
        if (this.allowedLogoutUrls.length === 0) {
            throw new Error(
                'allowedLogoutUrls must not be empty - logout flow will fail without valid logout URLs'
            )
        }
    ...
    private attachGroupPolicies(): void {
        ...
        const groupBasedPolicy = new iam.Policy(
            this,
            'GroupBasedAccessPolicy',
            {
                statements: [
    ...
    private createOutputs(): void {
        ...
        const domain = this.userPool.node.findChild('Domain') as
