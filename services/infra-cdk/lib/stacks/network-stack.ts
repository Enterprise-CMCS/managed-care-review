...
import { BaseStack, ImportedVpc, ServiceRegistry } from '@constructs/base'
import type { Construct } from 'constructs'
import * as ec2 from 'aws-cdk-lib/aws-ec2'
import { ResourceNames, INFRASTRUCTURE_SSM_PARAMS } from '@config/constants'
...
export class NetworkStack extends BaseStack {
    ...
    constructor(scope: Construct, id: string, props: NetworkStackProps) {
        super(scope, id, {
            ...props,
            description:
                'Network stack for Managed Care Review - imports existing VPC resources',
        })
    ...
    private createSecurityGroups(): void {
        ...
        this.lambdaSecurityGroup = new ec2.SecurityGroup(
            ...
            'LambdaSecurityGroup',
            {
                ...
                securityGroupName: ResourceNames.resourceName(
                    'lambda',
                    'sg',
                    this.stage
                ),
    ...
    private importVpnSecurityGroups(): void {
        ...
        try {
            ...
            if (
                vpnSecurityGroupId &&
                !vpnSecurityGroupId.includes('dummy-value')
            ) {
            ...
            console.info(
                `VPN security group not found in SSM, skipping import: ${error}`
            )
        }
        ...
        try {
            ...
            if (
                ...
                !sharedServicesGroupId.includes('dummy-value')
            ) {
            ...
            console.info(
                `Shared services security group not found in SSM, skipping import: ${error}`
            )
        }
    }
}
