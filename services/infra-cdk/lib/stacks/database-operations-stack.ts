...
import type { IVpc, ISecurityGroup, Instance } from 'aws-cdk-lib/aws-ec2'
...
export class DatabaseOperationsStack extends BaseStack {
    public pgToolsLayer: ILayerVersion
    public prismaEngineLayer: ILayerVersion
    public prismaMigrationLayer: ILayerVersion
    public dataExportBucket: IBucket
    public vmScriptsBucket: IBucket
    public postgresVm?: Instance
    private readonly vpc: IVpc
    private readonly lambdaSecurityGroup: ISecurityGroup
    private readonly databaseCluster: IDatabaseCluster
    private readonly databaseSecret: ISecret
    private readonly uploadsBucketName: string

    constructor(
        scope: Construct,
        id: string,
        props: DatabaseOperationsStackProps
    ) {
        super(scope, id, {
            ...props,
            description:
                'Database operations stack for Managed Care Review - Rotation, export/import, and management',
        })
    ...
    private createLambdaLayers(): void {
        ...
        const postgresToolsLayerArn = ServiceRegistry.getLayerArn(
            this,
            this.stage,
            'postgres-tools'
        )
        ...
        const prismaEngineLayerArn = ServiceRegistry.getLayerArn(
            this,
            this.stage,
            'prisma-engine'
        )
        ...
        const prismaMigrationLayerArn = ServiceRegistry.getLayerArn(
            this,
            this.stage,
            'prisma-migration'
        )
    ...
    private createSecretRotation(): void {
        ...
        if (config.security.secretRotation) {
            const secretRotation = new secretsmanager.SecretRotation(
                this,
                'SecretRotation',
                {
                    application:
                    ...
                    automaticallyAfter: Duration.days(
                        SECRETS_MANAGER_DEFAULTS.ROTATION_DAYS
                    ),
    ...
    private createDatabaseFunctions(): void {
        ...
        const dbManagerFunction = new BaseLambdaFunction(
            this,
            'DbManagerFunction',
            {
        ...
        const dbManagerIntegration = new LambdaToSecretsmanager(
            this,
            'DbManagerIntegration',
            {
                existingLambdaObj:
                    dbManagerFunction.function as lambda.Function,
        ...
        ;(dbManagerFunction.role as iam.Role).addToPolicy(
            new iam.PolicyStatement({
                ...
                resources: [
                    `arn:aws:secretsmanager:${this.region}:${this.account}:secret:aurora_${SERVICES.POSTGRES}_*`,
                ],
            })
        )

        // Database Export function - keeping current pattern (can convert later when handlers exist)
        const dbExportFunction = new BaseLambdaFunction(
            this,
            'DbExportFunction',
            {
                ...
                environment: {
                    S3_BUCKET:
                        this.dataExportBucket?.bucketName ||
                        `postgres-${this.stage}-dummy-export`,
                    DB_SECRET_ARN: this.databaseSecret.secretArn,
                    SECRETS_MANAGER_ENDPOINT: `https://secretsmanager.${this.region}.amazonaws.com`,
                    ...(config.features.enableCrossAccountRoles &&
        ...
        const dbImportFunction = new BaseLambdaFunction(
            this,
            'DbImportFunction',
            {
                ...
                environment: {
                    S3_BUCKET:
                        this.dataExportBucket?.bucketName ||
                        `postgres-${this.stage}-dummy-import`,
                ...
                layers: [
                    this.pgToolsLayer,
                    this.prismaEngineLayer,
                    this.prismaMigrationLayer,
                ],
                depsLockFilePath: path.join(
                    __dirname,
                    '..',
    ...
    private createPostgresVm(): void {
        ...
        const vmSecurityGroup = new ec2.SecurityGroup(
            this,
            'PostgresVmSecurityGroup',
            {
                vpc: this.vpc,
    ...
    private createCrossAccountRoles(): void {
        ...
        if (config.environment === 'val') {
            ...
            const crossAccountRole = new iam.Role(
                ...
                'CrossAccountUploadRole',
                {
                    ...
                    assumedBy: new iam.ArnPrincipal(
                        `arn:aws:iam::${AWS_ACCOUNTS.PROD}:root`
                    ),
                }
            )
            ...
            if (this.dataExportBucket) {
                this.dataExportBucket.addToResourcePolicy(
                    new iam.PolicyStatement({
    ...
    private grantDbManagerPermissions(func: BaseLambdaFunction): void {
        ...
        ;(func.role as iam.Role).addToPolicy(
            new iam.PolicyStatement({
                effect: iam.Effect.ALLOW,
                actions: [
                    'secretsmanager:DescribeSecret',
    ...
    private createProdExportRole(func: BaseLambdaFunction): void {
        ...
        prodExportRole.addToPolicy(
            new iam.PolicyStatement({
                effect: iam.Effect.ALLOW,
                actions: ['sts:AssumeRole'],
                resources: [
                    `arn:aws:iam::${AWS_ACCOUNTS.VAL}:role/${SERVICES.POSTGRES}-cross-account-upload-val${CDK_DEPLOYMENT_SUFFIX}`,
                ],
            })
        )

        // Replace the function's role
        ;(func.function.node.defaultChild as lambda.CfnFunction).role =
            prodExportRole.roleArn
    }
