...
import type { LAMBDA_FUNCTIONS } from '@config/constants'
import {
    SERVICES,
    API_PATHS,
    LAMBDA_MEMORY,
...
export class ApiComputeStack extends BaseStack {
    ...
    constructor(scope: Construct, id: string, props: ApiComputeStackProps) {
        super(scope, id, {
            ...props,
            description:
                'Combined API Gateway and Lambda compute stack for Managed Care Review',
        })
     ...
     */
    private getFunctionSpecificEnvironment(
        functionName: string
        ...
        const specificEnvs: Record<string, Record<string, string>> = {
            ...
            ZIP_KEYS: {
                MAX_TOTAL_SIZE: (
                    FILE_SIZE_LIMITS.MAX_SCAN_SIZE_BYTES * 3.5
                ).toString(), // ~1GB
                BATCH_SIZE: '50',
            },
        }
    ...
    private getFunctionSpecificConfig(
        ...
        const specificConfigs: Record<string, Partial<CreateFunctionProps>> = {
            ...
            MIGRATE: {
                ...
                additionalLayers: [this.getMigrationLayer()].filter(
                    Boolean
                ) as lambda.ILayerVersion[],
            },
            GRAPHQL: {
    ...
    private createLambdaFunctions(): void {
        ...
        Object.entries(this.FUNCTION_CONFIGS).forEach(
            ([functionName, config]) => {
                const specificEnv =
        ...
        this.functions.forEach((func) => {
            ...
            func.function.addToRolePolicy(
                new iam.PolicyStatement({
                    ...
                    resources: [
                        this.databaseSecretArn.replace(':secret:', ':cluster:'),
                    ],
                })
            )
    ...
    private createScheduledEvents(): void {
        ...
        cleanupRule.addTarget(
            new targets.LambdaFunction(cleanupFunction as lambda.Function)
        )
    }
    ...
    private createApiEndpoints(): void {
        ...
        const graphqlResource = this.api.root.addResource(
            API_PATHS.GRAPHQL.replace('/', '')
        )
        ...
        ApiEndpointFactory.createAuthenticatedBinaryEndpoint(
            this,
            'ZipKeysEndpoint',
            {
        ...
        externalResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(this.getFunction('GRAPHQL')),
            {
