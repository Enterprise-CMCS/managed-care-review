...
export class DataStack extends BaseStack {
    ...
    constructor(scope: Construct, id: string, props: DataStackProps) {
        super(scope, id, {
            ...props,
            description:
                'Data stack for Managed Care Review - Aurora database and S3 buckets',
        })
    ...
    private createDatabase(): void {
        ...
        if (!this.databaseSecurityGroup) {
            throw new Error(
                'Database security group not initialized in DataStack'
            )
        }
    ...
    private createS3Buckets(): void {
        ...
        this.qaBucket.addToResourcePolicy(
            new iam.PolicyStatement({
                sid: 'DenyUnsupportedFileTypes',
                effect: iam.Effect.DENY,
                principals: [new iam.AnyPrincipal()],
