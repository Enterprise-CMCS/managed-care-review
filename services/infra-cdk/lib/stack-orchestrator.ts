...
import type { GitHubOidcStack } from './stacks'
import {
    FoundationStack,
    NetworkStack,
    DataStack,
...
export class StackOrchestrator {
    ...
    createAllStacks(): StackReferences {
        ...
        const apiCompute = this.createApiCompute(
            foundation,
            network,
            data,
            auth,
    ...
    private createFoundation(): FoundationStack {
        return new FoundationStack(
            this.props.app,
            `MCR-Foundation-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
    ...
    private createNetwork(foundation: FoundationStack): NetworkStack {
        const stack = new NetworkStack(
            this.props.app,
            `MCR-Network-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createLambdaLayers(network: NetworkStack): LambdaLayersStack {
        const stack = new LambdaLayersStack(
            this.props.app,
            `MCR-LambdaLayers-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
    ...
    private createData(network: NetworkStack): DataStack {
        ...
        const stack = new DataStack(
            this.props.app,
            `MCR-Data-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createAuth(foundation: FoundationStack): AuthStack {
        ...
        try {
            ...
            console.info(
                `Email source address not found in SSM, will use default: ${error}`
            )
        }

        const stack = new AuthStack(
            this.props.app,
            `MCR-Auth-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createApiCompute(
        ...
        const stack = new ApiComputeStack(
            this.props.app,
            `MCR-ApiCompute-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
                ...
                postgresToolsLayerArn:
                    lambdaLayers.postgresToolsLayerVersionArn,
            }
        )
        stack.addDependency(foundation)
        stack.addDependency(network)
    ...
    private createDatabaseOps(
        ...
        const stack = new DatabaseOperationsStack(
            this.props.app,
            `MCR-DatabaseOps-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createFrontend(apiCompute: ApiComputeStack): FrontendStack {
        const stack = new FrontendStack(
            this.props.app,
            `MCR-Frontend-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createVirusScanning(
        ...
        const stack = new GuardDutyMalwareProtectionStack(
            this.props.app,
            `MCR-VirusScanning-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
    ...
    private createMonitoring(foundation: FoundationStack): MonitoringStack {
        const stack = new MonitoringStack(
            this.props.app,
            `MCR-Monitoring-${this.props.stage}${CDK_DEPLOYMENT_SUFFIX}`,
            {
                env: this.props.env,
                stage: this.props.stage,
                stageConfig: this.props.stageConfig,
