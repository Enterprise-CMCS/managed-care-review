...
export async function handler(event: any): Promise<any> {
    console.info(
        'GuardDuty rescan handler invoked:',
    ...
    for (const request of rescanRequests) {
        try {
            ...
            console.error(
                `Failed to rescan ${request.bucket}/${request.key}:`,
                error
            )
            results.failed++
            results.errors.push({
    ...
    console.info('Rescan results:', results)
    return results
}
...
async function rescanObject(request: RescanRequest): Promise<void> {
    ...
    console.info(
        `Initiating rescan for s3://${bucket}/${key}`,
    ...
    const lastScanStatus = existingTags.find(
        (tag) => tag.Key === 'GuardDutyMalwareScanStatus'
    )?.Value
    const lastScanTime = existingTags.find(
        (tag) => tag.Key === 'GuardDutyScanDate'
    )?.Value

    console.info(
        `Previous scan: Status=${lastScanStatus}, Time=${lastScanTime}`
    )
    ...
    console.info(`Created temporary copy at s3://${bucket}/${tempKey}`)
    ...
    console.info(`Copied back to original location, triggering new scan`)
