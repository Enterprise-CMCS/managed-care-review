...
export async function handler(
    ...
    try {
        ...
        const newTags: Tag[] = [
            ...
            {
                Key: 'virusScanTimestamp',
                Value: new Date(scanDate).getTime().toString(),
            },
        ]
        ...
        console.info(
            `Successfully tagged object s3://${bucketName}/${objectKey} with scan result: ${scanResult}`
        )
...
async function emitMetrics(
    ...
    console.info(
        `Scan completed - Result: ${scanResult}, Bucket: ${bucketName}`
    )
