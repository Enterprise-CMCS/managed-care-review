...
const BUCKETS_TO_MONITOR = (process.env.MONITORED_BUCKETS || '')
    .split(',')
    .filter(Boolean)
const METRICS_NAMESPACE =
    process.env.METRICS_NAMESPACE || 'MCR/VirusScanComparison'
...
export async function handler(event: ScheduledEvent): Promise<void> {
    console.info('Starting virus scan comparison', { event })
    ...
    do {
        ...
        if (listResponse.Contents) {
            ...
            for (const object of listResponse.Contents) {
                if (object.Key) {
                    const comparison = await compareObjectTags(
                        bucket,
                        object.Key
                    )
...
async function compareObjectTags(
    bucket: string,
    key: string
    ...
    try {
        ...
        const guardDutyTag = tags.find(
            (t) => t.Key === 'GuardDutyMalwareScanStatus'
        )
        const scanTimestamp = tags.find(
            (t) =>
                t.Key === 'virusScanTimestamp' || t.Key === 'GuardDutyScanDate'
        )
...
function analyzeResults(results: ComparisonResult[]): AnalysisResult {
    ...
    const missingClamAvCount = discrepancies.filter(
        (r) => r.discrepancyType === 'MISSING_CLAMAV'
    ).length
    const missingGuardDutyCount = discrepancies.filter(
        (r) => r.discrepancyType === 'MISSING_GUARDDUTY'
    ).length
    const statusMismatchCount = discrepancies.filter(
        (r) => r.discrepancyType === 'STATUS_MISMATCH'
    ).length
    ...
    return {
        ...
        guardDutyCoverage:
            totalCount > 0 ? (scannedByGuardDuty / totalCount) * 100 : 0,
        clamAvCoverage:
            totalCount > 0 ? (scannedByClamAv / totalCount) * 100 : 0,
    }
}
...
async function publishMetrics(metrics: MetricDatum[]): Promise<void> {
    ...
    console.info(`Published ${metrics.length} metrics to CloudWatch`)
}
