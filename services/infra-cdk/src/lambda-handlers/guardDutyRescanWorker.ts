...
export async function handler(
    ...
    console.info(
        `Processing ${event.keys.length} files in bucket ${event.bucket}`
    )
    ...
    for (const key of event.keys) {
        try {
            console.info(`Processing rescan for: ${key}`)
            ...
            const scanStatus = existingTags.find(
                (tag) =>
            ...
            console.info(
                `Current scan status for ${key}: ${scanStatus || 'NONE'}`
            )
            ...
            if (
                ...
                try {
                    ...
                    console.info(`Created temporary copy: ${tempKey}`)
                    ...
                    console.info(`Triggered rescan for: ${key}`)
                    ...
                    try {
                        ...
                        console.warn(
                            `Failed to clean up temp file ${tempKey}:`,
                            deleteError
                        )
                    }
                ...
                console.info(
                    `Skipping ${key} - already has valid scan status: ${scanStatus}`
                )
                processedKeys.push(key)
            }
        } catch (error) {
    ...
    console.info(
        `Rescan worker complete. Processed: ${processedKeys.length}, Failed: ${failedKeys.length}`
    )
