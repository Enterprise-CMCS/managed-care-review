...
export abstract class BaseStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: BaseStackProps) {
        ...
        const stackName = ResourceNames.stackName(
            props.serviceName,
            props.stage
        )
...
async function main() {
    ...
    const apiComputeStack = new ApiComputeStack(
        app,
        `MCR-ApiCompute-${stage}`,
        {
            env,
            stage,
            stageConfig,
...
export class AuroraServerlessV2 extends Construct {
    ...
    constructor(scope: Construct, id: string, props: AuroraServerlessV2Props) {
        ...
        this.dbCredentials = rds.Credentials.fromGeneratedSecret(
            'mcreviewadmin',
            {
        ...
        if (props.databaseConfig.secretRotationDays) {
            new secretsmanager.RotationSchedule(this, 'RotationSchedule', {
                ...
                automaticallyAfter: Duration.days(
                    props.databaseConfig.secretRotationDays
                ),
            })
        }
    ...
    constructor(scope: Construct, id: string, props: SecureS3BucketProps) {
        ...
        this.bucket = new s3.Bucket(this, 'Bucket', {
            ...
            cors: props.corsEnabled
                ...
                : undefined,
            serverAccessLogsPrefix: props.enableAccessLogging
                ? 'access-logs/'
                : undefined,
        ...
        if (props.allowedFileTypes) {
            const notAllowedTypes = props.allowedFileTypes.map(
                (ext) => `${this.bucket.bucketArn}/*${ext}`
            )
        ...
        if (props.lambdaTrigger) {
            this.bucket.addEventNotification(
                ...
                props.notificationPrefix
                    ? { prefix: props.notificationPrefix }
                    : undefined
            )
        }
