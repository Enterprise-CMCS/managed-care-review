...
import { LambdaLayers } from '../lib/stacks/lambda-layers'
...
async function main(): Promise<void> {
    try {
        ...
        new LambdaLayers(
            app,
            ResourceNames.stackName('lambda-layers', appConfig.stage),
            {
                env,
                stage: appConfig.stage,
                stageConfig: config,
                serviceName: 'lambda-layers',
            }
        )
        ...
        console.info(
            `CDK synthesis completed for Lambda Layers stack: ${appConfig.stage}`
        )
    } catch (error) {
        console.error('Lambda Layers stack initialization failed:', error)
        process.exit(1)
    }
}
