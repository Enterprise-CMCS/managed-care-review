...
describe('findContract', () => {
    ...
    it('finds a full contract', async () =>  {
        ...
        must(await updateContractDraft(client, contractA.id, 'a.1 body', [rate1.id, rate3.id]))
        ...
        expect(revisions[0].contractRevisions && revisions[0].contractRevisions[0].contractFormData).toBe('one contract')
        expect(revisions[0].submitInfo?.updatedReason).toBe('Rate Submit')

        expect(revisions[1].contractRevisions).toHaveLength(1)
        expect(revisions[1].contractRevisions && revisions[1].contractRevisions[0].contractFormData).toBe('one contract')
        expect(revisions[1].submitInfo?.updatedReason).toBe('1.1 new name')

        expect(revisions[2].contractRevisions).toHaveLength(1)
        expect(revisions[2].contractRevisions && revisions[2].contractRevisions[0].contractFormData).toBe('a.1 body')
