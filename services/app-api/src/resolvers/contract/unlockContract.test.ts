...
describe('unlockContract', () => {
    ...
    describe('Health plan contract tests', () => {
        describe.each(iterableCmsUsersMockData)(
            ...
            ({ mockUser }) => {
                ...
                it('returns status error if contract is actively being edited in draft', async () => {
                    ...
                    const unlockResult2 = await executeGraphQLOperation(
                        cmsServer,
                        {
                ...
                it('can unlock resubmit complex contracts to remove child rates', async () => {
                    ...
                    const contractA0 = await submitTestContract(
                        stateServer,
                        AID
                    )
        ...
        it('handles unlock and editing rates', async () => {
            ...
            const unlockedB = await fetchTestContract(
                stateServer,
                contractB0.id
            )
            ...
            expect(
                updatedB.draftRates![1].draftRevision?.formData
                    .rateDateCertified
            ).toBe('2000-01-22')
        })
    ...
    describe('EQRO contract tests', () => {
        describe.each(iterableCmsUsersMockData)(
            '$userRole unlockContract tests',
            ({ mockUser }) => {
                it('unlocks a EQRO contract and retains EQRO fields', async () => {
                    const stateServer = await constructTestPostgresServer({
                        s3Client: mockS3,
                    })
                    const cmsServer = await constructTestPostgresServer({
                    ...
                    const updateResult =
                        await createAndUpdateTestEQROContract(stateServer)
                    ...
                    expect(unlockedContract.draftRevision).toBeDefined()
                    expect(unlockedContract.status).toBe('UNLOCKED')
                    expect(unlockedContract.contractSubmissionType).toBe('EQRO')

                    if (!unlockedContract.draftRevision) {
                        throw new Error('Unexpected error: No draftRevision')
                    }
                    if (!unlockedContract.draftRevision.unlockInfo) {
                        throw new Error('Unexpected error: No unlockinfo')
                    }
                    ...
                    expect(unlockedFormData).toEqual(
                        expect.objectContaining({
