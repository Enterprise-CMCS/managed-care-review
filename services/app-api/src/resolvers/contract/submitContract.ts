            ...
            async (span) => {
                ...
                const updateResult = await store.updateDraftContractWithRates({
                    ...
                    formData: {
                        ...initialFormData,
                        managedCareEntities:
                            initialFormData.managedCareEntities,
                        stateContacts: initialFormData.stateContacts,
                        supportingDocuments:
                            initialFormData.supportingDocuments.map((doc) => {
                                return {
                                    name: doc.name,
                                    s3URL: doc.s3URL,
                                    sha256: doc.sha256,
                        ...
                        contractDocuments:
                            initialFormData.contractDocuments.map((doc) => {
                                return {
                                    name: doc.name,
                                    s3URL: doc.s3URL,
                                    sha256: doc.sha256,
                ...
                const submitContractResult = await store.submitContract({
                    ...
                    // `chip-submission-automation` LD flag — gates HEALTH_PLAN CHIP review determination.
                ...
                if (status === 'RESUBMITTED') {
                    ...
                    stateContractEmailResult = isEQRO
                        ...
                        : await emailer.sendResubmittedStateEmail(
                              submitContractResult,
                              updateInfo,
                              submitterEmails,
                              statePrograms
