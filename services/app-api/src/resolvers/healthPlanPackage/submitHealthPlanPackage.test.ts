...
describe.each(flagValueTestParameters)(
    ...
    ({ flagName, flagValue }) => {
        ...
        it('returns a StateSubmission if complete', async () => {
            ...
            expect(resultDraft.submissionDescription).toBe(
            ...
            expect(resultDraft.contractDateStart).toEqual(
                draft.contractDateStart
            )
            ...
            expect(resultDraft.contractDocuments).toEqual(
                draft.contractDocuments
            )

            expect(resultDraft.federalAuthorities).toEqual(
                draft.federalAuthorities
            )
        ...
        it('send CMS email to CMS if submission is valid', async () => {
            ...
            const currentRevision =
                submitResult?.data?.submitHealthPlanPackage?.pkg.revisions[0]
                    .node
