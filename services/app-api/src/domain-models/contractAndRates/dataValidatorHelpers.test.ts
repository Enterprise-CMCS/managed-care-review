                ...
                draftRates: [
                    {
                        ...
                        stateNumber: 5,
                        parentContractID:
                            'cb9a1ecb-cdb6-4ef2-956d-3fba8776cd8b',
                        packageSubmissions: [],
                        draftRevision: {
                ...
                draftRates: [
                    {
                        ...
                        reviewStatus: 'UNDER_REVIEW',
                        parentContractID:
                            '6ab1b4c0-f9d2-4567-958a-3123e98328e8',
                        packageSubmissions: [],
                        draftRevision: {
...
describe('EQRO parsing and validation', () => {
    ...
    describe('validateEQROContractDraftRevisionInput', () => {
        it('Successfully validates EQRO data', async () => {
            ...
            const expectedResult = {
                ...formData,
                contractDateEnd: undefined, // zod parsing preprocess nulls to undefined
            }
        ...
        it('Returns error for invalid EQRO data', async () => {
            const prismaClient = await sharedTestPrismaClient()
            const postgresStore = NewPostgresStore(prismaClient)
            const stateCode = 'FL'
            const formData: ContractDraftRevisionFormDataInput = {
            ...
            const validatedFormData = validateEQROContractDraftRevisionInput(
                formData,
                stateCode,
                postgresStore
            )
            ...
            if (!(validatedFormData instanceof Error)) {
                throw new Error(
                    'Unexpected error: Was expecting validateEQROContractDraftRevisionInput to return and error'
                )
            }

            // Expect invalid EQRO form fields
            expect(validatedFormData.issues[0]).toEqual(
                expect.objectContaining({
                    code: 'custom',
                    message:
                        'Program(s) in [imNotAValidProgramID,83d0e9d9-6592-439a-b46c-3235e8192fa0] are not valid FL programs',
                    path: ['programIDs'],
                })
            )

            expect(validatedFormData.issues[1]).toEqual(
                expect.objectContaining({
                    expected: 'undefined',
                    code: 'invalid_type',
                    path: ['dsnpContract'],
            ...
            expect(validatedFormData.issues[2]).toEqual(
                expect.objectContaining({
                    origin: 'string',
                    code: 'invalid_format',
                    format: 'email',
                    path: ['stateContacts', 0, 'email'],
    ...
    describe('parseAndUpdateEqroFields', () => {
        describe('when triggering questions have not changed', () => {
            it('returns form data unchanged', () => {
                ...
                const updateFormData = baseCurrentFormData()
        ...
        it('nullifies all EQRO fields when multiple triggering questions change', () => {
            const currentFormData = baseCurrentFormData({
                contractType: 'BASE',
                populationCovered: 'MEDICAID',
                managedCareEntities: ['MCO'],
    ...
    describe('parseEQROContract', () => {
        ...
        it('should return an error if EQRO contract has rates', async () => {
            ...
            const errMessages = parsedContract.issues.map((err) => err.message)
            expect(errMessages[0]).toContain(
                'EQRO submission cannot contain rates'
            )
        })
        ...
        it('should return an error if submissionType is not CONTRACT_ONLY', async () => {
            ...
            contract.draftRevision!.formData.submissionType =
                'CONTRACT_AND_RATES'
        ...
        it('should return an error if AMENDMENT + CHIP + MCO missing required EQRO fields', async () => {
            ...
            contract.draftRevision!.formData.populationCovered =
                'MEDICAID_AND_CHIP'
        ...
        it('should return an error if AMENDMENT + conditional fields missing', async () => {
            ...
            const stateCode = 'FL'
            const contract = createValidEQROContract({
                contractType: 'AMENDMENT', // AMENDMENT with MEDICAID + MCO
                populationCovered: 'MEDICAID',
                eqroProvisionMcoEqrOrRelatedActivities: true, // When eqroProvisionMcoEqrOrRelatedActivities is true, additional fields are required
                eqroProvisionMcoNewOptionalActivity: undefined,
                eqroProvisionNewMcoEqrRelatedActivities: undefined,
            })
        ...
        it('should return an error if invalid program IDs', async () => {
            ...
            const stateCode = 'FL'
            const contract = createValidEQROContract({
                programIDs: ['fake-program-id', 'another-fake-id'], // Add invalid program ID
            })
