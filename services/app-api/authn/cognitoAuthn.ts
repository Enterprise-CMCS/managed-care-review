...
import { StateUser, User } from '../../app-web/src/common-code/domain-models/user'
...
function userAttrDict(cognitoUser: CognitoIdentityServiceProvider.AdminGetUserResponse): { [key: string]: string } {
...
export async function userFromCognitoAuthProvider(authProvider: string): Promise<Result<User,Error>> {
        ...
        try {
            ...
            if (!('email' in attributes && 'given_name' in attributes && 'family_name' in attributes)) {
                return err(new Error('User does not have all the expected attributes'))
            }
