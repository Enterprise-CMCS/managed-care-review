...
`IDM` means to use CMS IDM as the auth provider. This is what is set in dev/val/qa/prod. Users will be redirected to login in IDM and then redirected back to our site. Cognito will create users in a special Okta user pool and then everything will work the same as if you were auth'd with `AWS_COGNITO` above.
...
### `CLOUDFRONT_CERT_ARN`, `CLOUDFRONT_DOMAIN_NAME`, `CLOUDFRONT_SB_DOMAIN_NAME`

Read by the infrastructure CDK deployments.

Used in dev/val/qa/prod to configure HTTPS certificates and hostnames.

### `API_DOMAIN_NAME`

Optionally configures a custom public domain for the App API. QA uses this with `CLOUDFRONT_CERT_ARN` to expose its external GraphQL endpoint through `api.mc-review-qa.onemac.cms.gov`.
...
- `stage`: The deployment stage (`dev`, `val`, `qa`, `prod`, or a review branch)
