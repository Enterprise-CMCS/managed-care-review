...
const findStatePrograms = (stateCode) => {
...
*/
const domainDateToProtoDate = (domainDate) => {
    if (!domainDate) {
        return undefined;
    }
    return {
...
const domainDateToProtoTimestamp = (domainDate) => {
    if (!domainDate) {
        return undefined;
    }
...
function domainEnumToProto(domainEnum, protoEnum) {
    if (!domainEnum) {
        return undefined;
    }
    const protoKey = domainEnumStringToProtoString(protoEnum[0], domainEnum);
    return protoEnum[protoKey];
}
/*
    Convert array of domain enums to proto enums
*/
function domainEnumArrayToProto(protoEnum, domainEnumArray) {
    const enums = [];
    for (const domainEnum of domainEnumArray) {
        const penum = domainEnumToProto(domainEnum, protoEnum);
        if (penum) {
            enums.push(penum);
        }
    }
...
const toProtoBuffer = (domainData) => {
    ...
    const literalMessage = {
        ...
        submittedAt: (isLockedHealthPlanFormData(domainData) &&
        ...
        populationCovered: domainEnumToProto(domainData.populationCovered, mcreviewproto.PopulationCovered),
        submissionType: domainEnumToProto(domainData.submissionType, mcreviewproto.SubmissionType),
        stateCode: domainEnumToProto(domainData.stateCode, mcreviewproto.StateCode),
        stateNumber: domainData.stateNumber,
        // eventually this will need to be an array of ids
        programIds: domainData.programIDs,
        contractInfo: {
            contractType: domainEnumToProto(domainData.contractType, mcreviewproto.ContractType),
            contractExecutionStatus: domainEnumToProto(domainData.contractExecutionStatus, mcreviewproto.ContractExecutionStatus),
            contractDateStart: domainDateToProtoDate(domainData.contractDateStart),
            contractDateEnd: domainDateToProtoDate(domainData.contractDateEnd),
            managedCareEntities: domainEnumArrayToProto(mcreviewproto.ManagedCareEntity, domainData.managedCareEntities),
            federalAuthorities: domainEnumArrayToProto(mcreviewproto.FederalAuthority, domainData.federalAuthorities),
            contractDocuments: domainDocsToProtoDocs(domainData.contractDocuments),
            contractAmendmentInfo: contractAmendmentInfo,
            statutoryRegulatoryAttestation: domainData.statutoryRegulatoryAttestation,
            statutoryRegulatoryAttestationDescription: domainData.statutoryRegulatoryAttestationDescription,
        },
        rateInfos: domainData.rateInfos && domainData.rateInfos.length
            ? domainData.rateInfos.map((rateInfo) => {
                 ...
                 * 1 and beyond.
                 */
                const combinedActuaries = rateInfo?.actuaryContacts.concat(rateInfo?.addtlActuaryContacts ?? []) ?? [];
                return {
                    id: rateInfo.id,
                    rateType: domainEnumToProto(rateInfo.rateType, mcreviewproto.RateType),
                    rateCapitationType: domainEnumToProto(rateInfo.rateCapitationType, mcreviewproto.RateCapitationType),
                    rateDateStart: domainDateToProtoDate(rateInfo.rateDateStart),
                    rateDateEnd: domainDateToProtoDate(rateInfo.rateDateEnd),
                    rateDateCertified: domainDateToProtoDate(rateInfo.rateDateCertified),
                    ...
                    rateCertificationName: generateRateName(domainData, rateInfo, findStatePrograms(domainData.stateCode)),
                    rateProgramIds: rateInfo.rateProgramIDs,
                    rateAmendmentInfo: rateInfo.rateAmendmentInfo && {
                        effectiveDateStart: domainDateToProtoDate(rateInfo.rateAmendmentInfo.effectiveDateStart),
                        effectiveDateEnd: domainDateToProtoDate(rateInfo.rateAmendmentInfo.effectiveDateEnd),
                    },
                    /**
                     ...
                     */
                    actuaryContacts: combinedActuaries.map((actuaryContact) => {
                        const firmType = domainEnumToProto(actuaryContact.actuarialFirm, mcreviewproto.ActuarialFirmType);
                        return {
                            contact: {
                    ...
                    actuaryCommunicationPreference: domainEnumToProto(rateInfo.actuaryCommunicationPreference, mcreviewproto.ActuaryCommunicationType),
                    packagesWithSharedRateCerts: rateInfo.packagesWithSharedRateCerts,
                };
            })
            : undefined,
        /**
...
//# sourceMappingURL=toProtoBuffer.js.map