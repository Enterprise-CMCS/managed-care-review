...
import { unlockedHealthPlanFormDataZodSchema, lockedHealthPlanFormDataZodSchema, } from './zodSchemas';
import { isLockedHealthPlanFormData, generateRateName, } from '../../healthPlanFormDataType';
import { toLatestProtoVersion } from './toLatestVersion';
import { findStatePrograms } from '../../healthPlanFormDataType/findStatePrograms';
export function replaceNullsWithUndefineds(obj) {
...
*/
const protoTimestampToDomain = (protoDateTime) => {
    if (!protoDateTime) {
        return undefined;
    }
...
function enumToDomain(protoEnum, key) {
...
*/
function protoEnumArrayToDomain(protoEnum, protoEnumArray) {
    if (!protoEnumArray) {
        return [];
    }
...
*/
function removeEnumPrefix(defaultValue, protoEnum) {
...
function parseProtoDocuments(docs) {
...
function parseSharedRateCerts(certs) {
    if (certs === null || certs === undefined) {
        return undefined;
    }
    return replaceNullsWithUndefineds(certs);
}
function parseContractAmendment(amendment) {
    ...
    const cleanProvisions = replaceNullsWithUndefineds(amendment.modifiedProvisions);
    return {
        modifiedProvisions: {
            inLieuServicesAndSettings: cleanProvisions.inLieuServicesAndSettings,
            ...
            modifiedMedicaidBeneficiaries: cleanProvisions.modifiedMedicaidBeneficiaries,
            modifiedRiskSharingStrategy: cleanProvisions.modifiedRiskSharingStrategy,
            modifiedIncentiveArrangements: cleanProvisions.modifiedIncentiveArrangements,
            modifiedWitholdAgreements: cleanProvisions.modifiedWitholdAgreements,
            modifiedStateDirectedPayments: cleanProvisions.modifiedStateDirectedPayments,
            modifiedPassThroughPayments: cleanProvisions.modifiedPassThroughPayments,
            modifiedPaymentsForMentalDiseaseInstitutions: cleanProvisions.modifiedPaymentsForMentalDiseaseInstitutions,
            modifiedMedicalLossRatioStandards: cleanProvisions.modifiedMedicalLossRatioStandards,
            modifiedOtherFinancialPaymentIncentive: cleanProvisions.modifiedOtherFinancialPaymentIncentive,
            modifiedEnrollmentProcess: cleanProvisions.modifiedEnrollmentProcess,
            modifiedGrevienceAndAppeal: cleanProvisions.modifiedGrevienceAndAppeal,
            modifiedNetworkAdequacyStandards: cleanProvisions.modifiedNetworkAdequacyStandards,
            modifiedLengthOfContract: cleanProvisions.modifiedLengthOfContract,
            modifiedNonRiskPaymentArrangements: cleanProvisions.modifiedNonRiskPaymentArrangements,
        },
    };
}
function parseActuaryContacts(actuaryContacts) {
    if (!actuaryContacts) {
        return [];
    }
    const parsedActuaryContacts = replaceNullsWithUndefineds(actuaryContacts).map((aContact) => {
        ...
        return {
            ...
            actuarialFirm: enumToDomain(mcreviewproto.ActuarialFirmType, aContact.actuarialFirmType),
            actuarialFirmOther: cleanContact.actuarialFirmOther,
        };
    });
    return parsedActuaryContacts;
}
function parseProtoRateAmendment(rateAmendment) {
    if (!rateAmendment) {
        return undefined;
    }
    return {
...
function parseRateCertificationName(rateCertificationName) {
...
const updateRateCertificationNames = (undifferentiatedFormData) => {
    if (undifferentiatedFormData.rateInfos &&
        ...
        undifferentiatedFormData.rateInfos.forEach((rateInfo, index) => {
            if (rateInfo && !rateInfo.rateCertificationName) {
                undifferentiatedFormData.rateInfos[index].rateCertificationName = generateRateName(undifferentiatedFormData, rateInfo, findStatePrograms(undifferentiatedFormData.stateCode));
            }
        });
    }
    return undifferentiatedFormData;
};
function parseRateInfos(rateInfos) {
    const rates = [];
    if (rateInfos.length > 0) {
        rateInfos.forEach((rateInfo) => {
            ...
            const rate = {
                ...
                rateCapitationType: enumToDomain(mcreviewproto.RateCapitationType, rateInfo?.rateCapitationType),
                ...
                rateCertificationName: parseRateCertificationName(rateInfo?.rateCertificationName),
                actuaryContacts: parseActuaryContacts(certifyingActuary),
                addtlActuaryContacts: parseActuaryContacts(additionalActuaries),
                actuaryCommunicationPreference: enumToDomain(mcreviewproto.ActuaryCommunicationType, rateInfo?.actuaryCommunicationPreference),
                packagesWithSharedRateCerts: parseSharedRateCerts(rateInfo.packagesWithSharedRateCerts) ?? [],
            };
            rates.push(rate);
        });
    }
...
function parsePartialHPFD(status, maybeUnlockedFormData) {
    if (status === 'DRAFT') {
        // cast so we can set status
        const maybeDraft = maybeUnlockedFormData;
        ...
        const parseResult = unlockedHealthPlanFormDataZodSchema.safeParse(maybeDraft);
        ...
        return updateRateCertificationNames(parseResult.data);
    }
    else if (status === 'SUBMITTED') {
        const maybeLockedFormData = maybeUnlockedFormData;
        maybeLockedFormData.status = 'SUBMITTED';
        const parseResult = lockedHealthPlanFormDataZodSchema.safeParse(maybeLockedFormData);
        if (parseResult.success === false) {
            console.warn('ERROR: attempting to parse state submission proto failed.');
            return new Error('ERROR: attempting to parse state submission proto failed');
        }
...
const toDomain = (buff) => {
    ...
    const { id, status, createdAt, updatedAt, submittedAt, stateCode, stateNumber, populationCovered, programIds, submissionType, riskBasedContract, submissionDescription, stateContacts, contractInfo, rateInfos, } = formDataMessage;
    ...
    const maybeUnlockedFormData = {
        ...
        populationCovered: enumToDomain(mcreviewproto.PopulationCovered, populationCovered),
        submissionType: enumToDomain(mcreviewproto.SubmissionType, submissionType),
        ...
        contractType: enumToDomain(mcreviewproto.ContractType, formDataMessage?.contractInfo?.contractType),
        contractExecutionStatus: enumToDomain(mcreviewproto.ContractExecutionStatus, formDataMessage?.contractInfo?.contractExecutionStatus),
        contractDateStart: protoDateToDomain(formDataMessage.contractInfo?.contractDateStart),
        contractDateEnd: protoDateToDomain(formDataMessage.contractInfo?.contractDateEnd),
        contractDocuments: parseProtoDocuments(formDataMessage.contractInfo?.contractDocuments),
        managedCareEntities: protoEnumArrayToDomain(mcreviewproto.ManagedCareEntity, formDataMessage?.contractInfo?.managedCareEntities),
        federalAuthorities: protoEnumArrayToDomain(mcreviewproto.FederalAuthority, formDataMessage?.contractInfo?.federalAuthorities),
        contractAmendmentInfo: parseContractAmendment(contractInfo?.contractAmendmentInfo),
        ...
        statutoryRegulatoryAttestation: contractInfo?.statutoryRegulatoryAttestation ?? undefined,
        statutoryRegulatoryAttestationDescription: contractInfo?.statutoryRegulatoryAttestationDescription ??
            undefined,
    };
    ...
    if (formDataResult instanceof Error) {
        console.warn('ERROR: attempting to parse state submission proto failed.', id);
    }
