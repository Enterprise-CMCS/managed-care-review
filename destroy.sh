...
read -r -a bucketList <<< "$(aws s3api list-buckets --output text --query 'Buckets[*].Name')"
filteredBucketList=()
set +e
for i in "${bucketList[@]}"
do
...
read -r -a filteredStackList <<< "$(aws cloudformation describe-stacks | jq -r ".Stacks[] | select(.Tags[] | select(.Key==\"STAGE\") | select(.Value==\"$stage\")) | .StackName"))"
